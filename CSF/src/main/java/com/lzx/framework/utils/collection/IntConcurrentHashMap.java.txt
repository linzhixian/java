 ...
 */
 public class IntConcurrentHashMap<V> extends ReentrantLock{
    /*
    ...
    public IntConcurrentHashMap(int initialCapacity, float lf) {
        loadFactor = lf;
...
@SuppressWarnings("unchecked")
    static final <V> IntConcurrentHashMap<V>[] newArray(int i) {
    return new IntConcurrentHashMap[i];
    }
    ...
    public V put(int key, V value, boolean onlyIfAbsent) {
        lock();
        try {
            int c = count;
            ...
            if (e != null) {
                ...
                if (next == null)
                    ...
                    for (HashEntry<V> p = e; p != lastRun; p = p.next) {
                        ...
                        newTable[k] = new HashEntry<V>(p.hash,
                                                         n, p.value);
                    }
                }
            }
            ...
            if (e != null) {
                ...
                if (value == null || value.equals(v)) {
                    ...
                    for (HashEntry<V> p = first; p != e; p = p.next)
                        newFirst = new HashEntry<V>(p.hash,
                                                      newFirst, p.value);
                    tab[index] = newFirst;
                    count = c; // write-volatile
                }
...
//           this.key = key;
     ...
     abstract class HashIterator {
         ...
         public void remove() {
             ...
             IntConcurrentHashMap.this.remove(lastReturned.hash);
             lastReturned = null;
         }
     }
 	...
 	implements ReusableIterator<Integer>, Enumeration<Integer>
     {
         public Integer next()        { return super.nextEntry().hash; }
         public Integer nextElement() { return super.nextEntry().hash; }
     }
 	...
 	extends HashIterator
 	implements ReusableIterator<IntHashMap.Entry<V>>
     {
     ...
     final class KeySet extends AbstractSet<Integer> {
         ...
         public boolean contains(Object o) {
             if (o instanceof Integer)
                 return IntConcurrentHashMap.this.containsKey((Integer)o);
             return false;
         }
         public boolean remove(Object o) {
             if (o instanceof Integer)
                 return IntConcurrentHashMap.this.remove((Integer)o) != null;
             return false;
         }
         public void clear() {
     ...
     final class EntrySet extends AbstractSet<IntHashMap.Entry<V>> {
         ...
         public boolean remove(Object o) {
             if (!(o instanceof IntHashMap.Entry))
                 return false;
             IntHashMap.Entry<?> e = (IntHashMap.Entry<?>)o;
             return IntConcurrentHashMap.this.remove(e.getKey(), e.getValue()) != null;
         }
         public int size() {
             return IntConcurrentHashMap.this.size();
         }
